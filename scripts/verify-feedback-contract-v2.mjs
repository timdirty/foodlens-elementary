...
if (args.length && !(args.length === 2 && args[0] === "--container")) {
  throw new Error(
    "Usage: node scripts/verify-feedback-contract-v2.mjs [--container NAME]",
  );
}
...
const tests = ["evidence_chain", "rls", "retention"].map(
...
const sourcePaths = [
  ...
  "lib/__tests__/supabase-evidence-payload.test.ts",
];
...
const probe =
  "select jsonb_build_object('migration_count',(select count(*) from supabase_migrations.schema_migrations),'feedback_gate',to_regprocedure('public.foodlens_feedback_contract_version()'),'feedback_validator',to_regprocedure('private.validate_feedback_contract_v2(jsonb,jsonb,jsonb,integer)'),'meal_batches',(select count(*) from public.meal_batches),'contexts',(select count(*) from public.meal_contexts),'feedback',(select count(*) from public.feedback_events));";
const before = sql(probe);
if (before.status !== 0)
  throw new Error(before.stderr || "Baseline unavailable");
const baseline = JSON.parse(before.stdout.trim());
if (baseline.feedback_gate !== null || baseline.feedback_validator !== null)
  throw new Error(
    "This transaction replay requires the pre-v2 baseline; choose a separate baseline container. It never removes an existing migration.",
  );
